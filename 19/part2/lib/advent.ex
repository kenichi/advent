defmodule Advent do
  ...
  # defstruct registers: {1, 0, 0, 0, 0, 0},
  defstruct registers: {10551300, 10551300, 0, 3, 10551300, 10551300},
            instructions: [],
            count: 100,
            ip: 0
  ...
  def exec(%Advent{count: c} = state) do
    %Advent{state | count: c - 1}
    |> log()
    |> exec(inst(state))
    |> case do
      {:halt, s} -> s
      {:cont, %Advent{count: c} = s} when c < 0 -> s
      {:cont, s} ->
        s
        |> incr()
        |> exec()
