defmodule Advent do
  ...
  def eval(passes \\ Advent.Input.read()) do
    seat_ids = Enum.map(passes, &seat_id/1)

    between = fn x -> x not in seat_ids and (x + 1) in seat_ids and (x - 1) in seat_ids end

    for id <- 0..Enum.max(seat_ids), between.(id), do: Integer.to_string(id)
  end
