defmodule Advent do
  ...
  def remove_crashers(carts, rails) do
    carts -- 
      Enum.reduce(carts, [], fn c, crashed ->
        [ detect_crash(c, carts, rails) | crashed ]
      end)
  end

  def detect_crash(cart, carts, rails) do
    other_carts = carts -- [cart]
    new_cart = move(cart)

    Enum.find(other_carts, fn c ->
      new_other = move(c)
      new_cart.x == new_other.x && new_cart.y == new_other.y
    end)
  end

  def last_cart?({carts, _}) when length(carts) == 1 do
    cart = hd(carts)
    throw {cart.x, cart.y}
  end
  def last_cart?({carts, rails}), do: {carts, rails}
