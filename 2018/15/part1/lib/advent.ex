defmodule Advent do
  ...
  def get_unit({elves, goblins, _map}, {x, y}) do
    unit? = fn {ex, ey, _} -> ex == x && ey == y end
    cond do
  ...
  def print_map({_, _, map} = state, overload \\ nil, io \\ :stdio) do
    coords = Map.keys(map)
  ...
  def friends_enemies({elves, goblins, _}, unit) do
    cond do
  ...
  def attackable({_, _, map}, {ux, uy, _}) do
    [
      { ux + 1, uy },
      { ux - 1, uy },
      { ux, uy + 1 },
  ...
  def attack!({tx, ty, thp} = target, {elves, goblins, map}, rest) do
    # IO.puts("\tattacking #{inspect(target)}")

    find = fn e -> e == target end
    cond do
      (thp - 3) <= 0 ->
        if Enum.member?(elves, target) do
          { :stop,
            {
  ...
  def move({elves, goblins, map} = state, {_, _, hp} = unit, goal) do
  ...
  def do_round({elves, goblins, _map} = state) do
    elves ++ goblins
    |> reading_order
    |> do_round(state)
  end
  ...
  def remaining_hp({elves, goblins, _}) do
    elves ++ goblins
